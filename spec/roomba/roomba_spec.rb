...
describe "Roomba" do
  ...
  it "should test sample data" do
    engine = Roomba::Engine.new
    expect(Roomba.roam(engine.set_commands(Roomba::Commands.new(File.open("spec/fixtures/sample_commands_1.txt", "r:utf-8").readlines)))).to eq(["0,1,NORTH"])
    expect(Roomba.roam(engine.set_commands(Roomba::Commands.new(File.open("spec/fixtures/sample_commands_2.txt", "r:utf-8").readlines)))).to eq(["0,0,WEST"])
    expect(Roomba.roam(engine.set_commands(Roomba::Commands.new(File.open("spec/fixtures/sample_commands_3.txt", "r:utf-8").readlines)))).to eq(["3,3,NORTH"])
  end
end
